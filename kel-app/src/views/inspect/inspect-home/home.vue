...
const getNoFromQR = async deviceNO => {
  const basicParams = {
    ...
    patrolStatus: '0', // 未查询
    // 哥哥，为什么不传日期啊？？
    patrolDate: dayjs().format('YYYY-MM-01'), // 按月份查询
  };
...
const getCount = computed(() => {
  switch (tabActive.value) {
    case 'regulator':
      return [useMainData.getTyxNotDoneCount, useMainData.getTyxDoneCount];
    case 'well':
      return [useMainData.getFjNotDoneCount, useMainData.getFjDoneCount];
    default:
      return [0, 0];
  }
});
...
const getList = debounce(async (keyword, conditions) => {
  if (tabActive.value === 'yinbao') {
    ...
    destroy();
  } else if (['regulator', 'well'].includes(tabActive.value)) {
    finished.value = true;
    const type = tabActive.value === 'regulator' ? 'tyx' : 'fj';

    list.value = listDataAdapter.value(useMainData.getPointsByType(type)).filter(item => {
      return keyword ? item.pointCode.includes(keyword) : true;
    });

    taskCount.value = getCount.value[0];
    finishCount.value = getCount.value[1];
  }
});
...
const addRecord = computed(() => {
  switch (tabActive.value) {
    case 'regulator':
      return api_regulator.addPost;
    case 'well':
      return api_well.addPost;
    case 'pipeline':
      return list_pipeline_adapter;
    case 'yinbao':
      return list_yinbao_adapter;
...
const handleItemClick = async data => {
  ...
  const res = await apiList.value({
    patrolPlanId: data.patrolPlanId,
    equipmentsyscode: data.pointSyscode,
    delegateStatus: '1',
  });

  const delegateData = res.data.rows[0];
  ...
  if (delegateData) {
    // 转派
    router.push(
      `/inspect/checking/${delegateData.id}?type=${tabActive.value}&formType=edit&isDelegate=true`
    );
  } else {
    const res = await addRecord.value(data);

    const id = res.data.data;

    // 这里不需要签到
    router.push(`/inspect/checking/${id}?type=${tabActive.value}&formType=edit`);
  }
};
...
const handleNavigation = async data => {
  const { y: latitude, x: longitude } = data;
  ...
  if (tabActive.value === 'regulator') {
    addr += `调压箱${data.pointCode}`;
  }

  if (tabActive.value === 'well') {
    addr += `阀井${data.pointCode}`;
  }
...
const apiTrans = computed(() => {
  switch (tabActive.value) {
    ...
    case 'yinbao':
      return api_yinbao.transTaskAnother;
    default:
      return api_regulator.transTaskAnother;
  }
});
...
async function handleSearchNearby() {
  const key = tabActive.value === 'regulator' ? 'tyx' : tabActive.value === 'well' ? 'fj' : null;
  if (key) {
    await useMainData.sortByDistance(key);
    list.value = listDataAdapter.value(useMainData.getPointsByType(key));
  }
}
</script>
...
<template>
  <div class="main-container">
    ...
    <div v-if="isListShow" class="search-condition-container">
      ...
      >
        <template v-if="tabActive === 'regulator' || tabActive === 'well'" #common-search-left>
          <div class="nearby" @click="handleSearchNearby">查询附近</div>
        </template>
      </common-search-composition>
    ...
    <div v-if="isListShow" class="list-container" :style="listContainerStyle">
      ...
      >
        <task-item
          ...
          :needNavigation="tabActive === 'regulator' || tabActive === 'well'"
        />
      </van-list>
    </div>
...
.main-container {
  ...
  .nearby {
    background: url('/src/assets/image/check/icon-loc-look.png') no-repeat left/16px 16px;
    color: #34394f;
    padding-left: 20px;
    font-size: 16px;
    height: 24px;
