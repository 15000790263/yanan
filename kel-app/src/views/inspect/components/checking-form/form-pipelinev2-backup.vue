...
const handleStartCheck = async () => {
  ...
  showDialog({
    title: '提示',
    message: '巡检过程中可切换至调压箱、阀井、调压站进行巡检',
...
const planList = computed(() => {
  if (currentInspectWay.value === '0') {
    return useMainData.allPlan['gx']['plans'];
  } else {
    return [...useMainData.allPlan['jgcl']['plans'], ...useMainData.allPlan['kxdr']['plans']];
  }
});
        ...
        return {
          ...
          callback: async () => {
            ...
            if (res.data.rows.length) {
              ...
              router.push(
                `/inspect/checking/${res.data.rows[0].id}?type=regulator&formType=edit&isfrompipeline=1`
              );
            }
          },
        };
  ...
  if (valveList.value.length) {
    list.push({
      ...
      list: valveList.value.map(item => {
        return {
          text: item.valvewellcode,
          value: item.guid,
          callback: async () => {
            const res = await api_well.getList({
              patrolStatus: 0,
              planStatus: 1,
              patrolDate: dayjs().startOf('month').format('YYYY-MM-DD'),
              equipmentsyscode: item.guid,
            ...
            if (res.data.rows.length) {
              ...
              router.push(
                `/inspect/checking/${res.data.rows[0].id}?type=well&formType=edit&isfrompipeline=1`
              );
            }
          },
        };
