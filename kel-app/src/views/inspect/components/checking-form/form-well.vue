...
import { api_well, getStaffs, api_plan } from '@/api/inspect';
...
const handleConfirm = async () => {
  ...
  const res = await api_well.putCheckForm(params, fileBlobs, pictureCache).finally(() => {
    destroy();
  });
  ...
  if (res.status === 200 && res.data.code === 200) {
    ...
    if (getPatrolStatus(formData.value).patrolStatus === 2) {
      showConfirmDialog({
        ...
        .then(() => {
          router.push('/inspect/inspect-pitfall-add?source=well');
        })
        .catch(() => {
          isFromPipe();
          router.goBack();
...
const handleClickTransConfirm = async () => {
  const res = await api_well.transTaskAnother(
    props.data.id,
...
const handleCancel = () => {
  ...
  if (!editHistory.value && route.query.isDelegate !== 'true') {
    if (route.query.type === 'well') {
      api_well.deletePost(props.data.id);
    }
  }
};
...
<template>
  <div class="main-container">
    ...
    <div class="card">
      <basic-info type="well" :data="props.data" v-bind="$attrs">
        <template #icon>
          <div>
            <div
              style="
    ...
    <div class="card">
      <check-item
        label="有无泄露"
        allowPhoto
        :showPhoto="formData['leakageStatus'] !== '1'"
        @onPhotoClick="handleItemRecord('泄露')"
      >
      ...
      >
        <van-radio-group v-model="formData['wellItems']" shape="dot" direction="horizontal">
      ...
      >
        <van-radio-group v-model="formData['wellOccupy']" shape="dot" direction="horizontal">
      ...
      >
        <van-radio-group v-model="formData['compensator']" shape="dot" direction="horizontal">
