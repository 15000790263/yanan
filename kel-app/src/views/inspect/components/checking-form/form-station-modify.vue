  ...
  <div class="main-container-2" :class="{ 'is-modify': !!id }">
      ...
      <template v-for="item in itemList" :key="item.field">
        ...
        >
          <div class="check-wrapper">
            <van-radio-group v-model="formData[item.field]" shape="dot" direction="horizontal">
            ...
            >
              ...
              <template v-if="item.key === 'needBtn'" #button>
                <div
                  ...
                  :style="{ 'background-color': '#2ab1b8' }"
                  @click="formData[item.field] = ''"
                >
                  无
                </div>
              </template>
    ...
    <van-popup v-model:show="showPup" position="bottom" :style="{ height: '30%' }">
      <div class="pop-content">
        <ul @click="stationClick">
          <li
            :class="{ active: item.equipment === formData['stationName'] }"
            v-for="item in allStations"
            :key="item.dataid"
            :data-equipmentcode="item.equipmentcode"
          >
            {{ item.equipment }}
          </li>
        </ul>
...
const id = route.query.stationId;
...
onMounted(async () => {
  // 修改
  if (id) {
    const res = await api_station.getDetail(id);
