...
import { api_pipeline, api_regulator, api_well, api_plan } from '@/api/inspect';
...
const handleConfirm = async () => {
  ...
  showConfirmDialog({
    ...
    .then(async () => {
      ...
      // 处理巡检计划
      await api_pipeline
        .updatePlanCoverage(currentPlan.value.id, coverRate.value, coverLength.value)
        .finally(() => destroy());
...
const handleStartCheck = async () => {
  ...
  await locationStore.addPipelineData(currentPlan.value);
...
const handleCancel = () => {
  showConfirmDialog({
    ...
    .then(async () => {
      ...
      if (currentInspectWay.value === '1') {
        // 删除快巡的数据
        // 1、删除轨迹点
        api_plan.deleteByBatchCode(locationStore.checkId);
        // 2、删除记录
        await api_plan.deleteByBatchCode2(locationStore.checkId);

        // 3、修改完成数量
        const res2 = await api_plan.getKxRecordPointList({ patrolPlanId: currentPlan.value.id });
        await api_plan.updatePlanCnt({
          planId: currentPlan.value.id,
          completedCnt: res2.data.total,
        });
      }
...
function saveEndState() {
  if (isStarting.value) {
    ...
    locationStore.savaData({
      ...
      currentInspectWay: currentInspectWay.value,
    });
  }
}
...
const recoverSavedData = () => {
  ...
  if (isInspect) {
    ...
    currentInspectWay.value = cache.currentInspectWay;
...
// 快巡点数据
const kxPoints = computed(() => {
  if (currentInspectWay.value === '1') {
    return useMainData
      .getPointsByType('kx')
      .filter(item => item.patrolPlanId === currentPlan.value.id);
  } else {
    return [];
  }
});
...
async function showEarth() {
  ...
  // 显示快巡点
  const kxPoints2 = kxPoints.value.map(item => ({
    image: new URL('/src/assets/image/check/icon-kx.gif', import.meta.url).href,
    ...item,
    callback(marker) {
      kxMarkers.push({ id: item.pointId, marker });
    },
  }));
  earth.addBillboards(kxPoints2);

  if (kxPoints.value.length) {
    const arr = kxPoints.value.map(item => [item.y, item.x]);
    earth.viewer.fitBounds(arr);
  }
...
async function getCoverRate() {
  if (currentInspectWay.value === '0') {
    try {
      ...
      coverLength.value = l.toFixed();
    } catch (error) {
      coverRate.value = 0;
      console.error(error);
    }
  } else {
    const updateKxMarker = async () => {
      const res2 = await api_plan.getKxRecordPointList({ patrolPlanId: currentPlan.value.id });

      // 更新快巡红色标记为绿色
      res2.data.rows.forEach(item => {
        const marker = kxMarkers.find(m => m.id == item.patrolpointNumber);
        if (marker) {
          marker.marker.setIcon(
            L.icon({
              iconUrl: new URL('/src/assets/image/check/icon-kx-finish.gif', import.meta.url).href,
              iconSize: [32, 32],
              iconAnchor: [16, 32],
            })
          );
    ...
    const res = await api_plan.getKxFinishPoint({
      planId: currentPlan.value.id,
      groupName: currentPlan.value.groupName,
    });
    ...
    if (res.data.length) {
      const data = res.data.map(item => {
        return {
          ...
          lon: item.longitude,
          lat: item.latitude,
          patrolPlanId: currentPlan.value.id,
          patrolpointNumber: item.point_id,
          batchCode: locationStore.checkId,
        };
      });

      await api_plan.postKxRecordPoint(data).catch(err => {
        console.log(err);
      });

      const res2 = await updateKxMarker();

      coverRate.value = ((res2.data.total * 100) / (currentPlan.value.totalCount || 100)).toFixed();

      coverLength.value = res2.data.total;
...
const inspectWay = computed(() => {
  return inspectWayList.value[currentInspectWay.value].label;
});
...
const hasInspectTitle = computed(() => {
  return currentInspectWay.value === '0' ? '已巡检长度(m)' : '已巡检数量';
});
...
<template>
  <div class="main-container-2">
    <div class="card form">
      ...
      <check-item v-if="isStarting" :label="hasInspectTitle" :labelOneLine="true">
        <div class="input-container">
          <van-field
            border
