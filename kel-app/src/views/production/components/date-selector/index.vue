<template>
  <div>
    ...
    <van-popup v-else v-model:show="showBottom" position="bottom" :style="{ height: '40%' }">
      <van-date-picker
        v-model="currentDate"
        title="选择"
        :max-date="dayjs(lastDay).add(1, 'day').toDate()"
...
watch(
  ...
  value => {
    ...
    switch (strV) {
      ...
      case '["year","month","day"]':
        currentDate.value = [
          dayjs(props.lastDay).year(),
          dayjs(props.lastDay).month() + 1,
          dayjs(props.lastDay).date() + 1,
        ];
