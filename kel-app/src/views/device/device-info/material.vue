            ...
            <div class="content">
              <div class="content-list">
                ...
                >
                  <ul class="pipeline-list">
                    <template v-for="(item, index) in materialList" :key="index">
                      <li v-if="value.id === 6012" @click="handleItemClick(item, 6012)">
                        <van-row gutter="20">
                          <van-col span="24">
                          ...
                          <van-col span="24">
                            <div class="title">购置日期：</div>
                            <div class="cont">{{ item.acquisitiondatetime }}</div>
                          </van-col>
                          <van-col span="24">
                            <div class="title">使用性质：</div>
                            <div class="cont">
                              {{ sysStore.getDictValue('natrue_use', item.natrueuse) }}
                            </div>
                          </van-col>
                          <van-col span="24">
                            <div class="title">负责人：</div>
                      ...
                      <li v-if="value.id === 5021" @click="handleItemClick(item, 5021)">
                        <van-row gutter="20">
                          <van-col span="24">
                      ...
                      <li v-if="value.id === 4699" @click="handleItemClick(item, 4699)">
                        <van-row gutter="20">
                      ...
                      <li v-if="value.id === 5029" @click="handleItemClick(item, 5029)">
                        <van-row gutter="20">
                          <van-col span="24" v-if="item.equipment">
                            <div class="title">设备名称：</div>
                            <div class="cont">{{ item.equipment }}</div>
                      ...
                      <li v-if="value.id === 5037" @click="handleItemClick(item, 5037)">
                        <van-row gutter="20">
                          <van-col span="24" v-if="item.equipment">
                            <div class="title">设备名称：</div>
                            <div class="cont">{{ item.equipment }}</div>
...
<script setup>
import { getMaterialList } from '../../../api/device';
...
const tabList = ref([
  ...
  { text: '检测设备', id: 5021 },
  {
    ...
    id: 5023,
    children: [
      { text: '救援物资', value: 5023 },
      { text: '备品备件', value: 7011 },
    ],
  },
  { text: '电气设备', id: 4699 },
  { text: '通讯设备', id: 5029 },
...
function getData() {
  getMaterialList({ equipmenttypeid, pageSize, pageNum, url }).then(res => {
...
const map = {
  6012: '/baseequipmentspecial/baseequipmentspeciallist',
  5021: '/basemeasuringinstrument/basedetectioninstrumentlist',
  5023: '/fixedasset/list',
  7011: '/equipmentspareparts/equipmentsparepartslist',
  4699: '/basemeasuringinstrument/basedetectioninstrumentlist',
  5029: '/basemeasuringinstrument/basedetectioninstrumentlist',
  5037: '/baseEquipmentBlind/baseEquipmentBlindList',
};
...
function handleSearch() {
  ...
  if (value) {
    getMaterialList({ equipmenttypeid, pageSize: 10, pageNum: 1, url, equipment: value }).then(
      res => {
        initParams();
        finished.value = true;
        materialList.value.push(...res.data.rows);
...
function handleItemClick(item, typeValue) {
  ...
  router.push(
    `/device/material-detail?dataId=${dataid}&typeValue=${typeValue}&equipmentcode=${equipmentcode}&equipment=${equipment}&equipmentsyscode=${equipmentsyscode}&espId=${espId}`
  );
}
</script>
