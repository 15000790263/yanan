...
async function getDetail() {
  const res = await getTYQDetailByDataId(dataId);
  const {
    equipmentcode,
    compressorboxlmodel,
    configurationtype,
  ...
  main5001.equipmentcode = equipmentcode;
  main5001.compressorboxlmodel = compressorboxlmodel;
  main5001.configurationtype = configurationtype;
  main5001.flowdesign = flowdesign;
  main5001.desiginletfressure = desiginletfressure;
  main5001.desienlutletpressure = desienlutletpressure;
  main5001.maxinletpressure = maxinletpressure;
  main5001.maxoutletpressure = maxoutletpressure;
  main5001.inletdiameter = inletdiameter;
  main5001.outletdiameter = outletdiameter;
  main5001.manufactureddate = manufactureddate;
  main5001.status = status;
  main5001.regionid = regionid;
  main5001.x = x;
  main5001.y = y;
}
...
async function onConfirm() {
  ...
  const data = { ...main5001, dataid: dataId };
  const {
    compressorboxgrade,
    equipmentcode,
    compressorboxlocation,
    compressorboxroad,
    x,
    y,
    netname,
          ...
          children: [
            {
              ...
              _: (() => {
                if (originNetList && main5001?.regionid) {
                  const id = main5001?.regionid.split(',').pop();

                  const item = originNetList.find(i => i.id === id);
                  const p = originNetList.find(i => i.id === item.pid);

                  fieldValue.value = `${netList.value[0].netname}/${p.netname}/${item.netname}  ${item.netlevel}`;
                }
              })(),
            },
