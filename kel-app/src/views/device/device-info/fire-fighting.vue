  ...
  <div class="content">
    <div class="content-list">
      <van-list v-model:loading="loading" finished-text="没有更多了" :finished="finished" :immediate-check="false"
        ...
        <ul class="pipeline-list">
          <template v-for="(item, index) in materialList" :key="index">
                      <li v-if="value.id === 5034" @click="handleItemClick(item, 5034)">
                        <van-row gutter="20">
                          <van-col span="24">
                          ...
                          <van-col span="24">
                            ...
                            <div class="cont">{{ item.outfactorynumber }}</div>
                          </van-col>
                          <van-col span="24">
                            <div class="title">规格型号：</div>
                            <div class="cont">{{ item.specmodel }}</div>
                          ...
                          <van-col span="24">
                            ...
                            <div class="cont">{{ item.installplace }}</div>
                          </van-col>
                          <van-col span="24">
...
const map = {
  5034: '/baseEquipmentExtinguisher/baseEquipmentExtinguisherList',
};
...
function handleSearch() {
  ...
  if (value) {
    getProtectionsList({ equipmenttypeid, pageSize: 10, pageNum: 1, url, equipment: value }).then(
      res => {
        initParams();
        finished.value = true;
        materialList.value.push(...res.data.rows);
...
function handleItemClick(item, typeValue) {
  ...
  console.log(item)
  router.push(
    `/device/fire-fighting-detail?dataId=${dataid}&typeValue=${typeValue}&equipmentcode=${equipmentcode}&equipment=${equipment}&equipmentsyscode=${equipmentsyscode}&espId=${espId}`
  );
}
</script>
