  ...
  <div class="content">
    <div class="content-list">
      <van-list v-model:loading="loading" finished-text="没有更多了" :finished="finished" :immediate-check="false"
        ...
        <ul class="pipeline-list">
          <template v-for="(item, index) in materialList" :key="index">
                      <li v-if="value.id === 7008" @click="handleItemClick(item, 7008)">
                        <van-row gutter="20">
                          <van-col span="24">
...
const map = {
  7008: '/baseInsulatedJoint/baseInsulatedJointList',
};
...
function handleItemClick(item, typeValue) {
  ...
  console.log(item)
  router.push(
    `/device/isolation-detail?dataId=${dataid}&typeValue=${typeValue}&equipmentcode=${equipmentcode}&equipment=${equipment}&equipmentsyscode=${equipmentsyscode}&espId=${espId}`
  );
}
</script>
